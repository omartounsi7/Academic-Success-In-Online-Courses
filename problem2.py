...
def regress(X_train, y_train, X_test, y_test):
    ...
    reg = linear_model.Ridge(alpha=10, fit_intercept=True)
...
if __name__ == '__main__':
    ...
    ids, avg_scores, avg_features = getIdsAndScores()
    avg_features = np.array(avg_features)
    avg_scores = np.array([avg_scores])
    print("shape of features: ", avg_features.shape, "shape of scores: ", avg_scores.transpose().shape)
    data = np.append(avg_features, avg_scores.transpose(), axis=1)
    #data = np.delete(data,[0, 1, 2, 5, 6, 7, 8], 1)
    #print("data, ", data)
    ...
    print("before round mse: ", mean_squared_error(y_test, y_pred))
    print("before round r2: ", r2_score(y_test, y_pred))
